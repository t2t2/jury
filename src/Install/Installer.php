...
// This is the default ProcessWire installer, adjusted for composer use
...
class Installer {
	...
	public function execute() {
		...
		require($this->config->paths->wire . "modules/AdminTheme/AdminThemeDefault/install-head.inc"); 
		...
		require($this->config->paths->wire . "modules/AdminTheme/AdminThemeDefault/install-foot.inc"); 
	}
	...
	protected function initProfile() {
		...
		if(is_file("{$this->config->paths->site}install/install.sql")) {
			$this->ok("Found installation profile in /site/install/");

		} else if(is_dir($this->config->paths->site)) {
			$this->ok("Found /site/ -- already installed? ");

		} else {
			$this->err("Please copy a processwire profile into the /site/ folder.");
			return;
		}
	...
	protected function compatibilityCheck() { 
		...
		$dirs = array(
			// directory => required?
			$this->config->paths->public . substr($this->config->urls->assets, strlen($this->config->urls->root)) => true,
			$this->config->paths->siteModules => false, 
		);
		...
		if(is_writable($this->config->paths->root . "config.php")) $this->ok("/config.php is writable"); 
			else $this->err("/config.php must be writable. Please create it / adjust the server permissions before continuing."); 
		
		if(!strpos(file_get_contents($this->config->paths->public . ".htaccess"), "PROCESSWIRE")) {
	...
	protected function dbConfig($values = array()) {
		...
		if(!is_file($this->config->paths->site . "install/install.sql")) die("There is no installation profile in /site/. Please place one there before continuing. You can get it at processwire.com/download"); 
		...
		echo "</select></p><br style='clear: both;' />";
		echo "</div>";
	...
	protected function dbSaveConfigFile(array $values) {
		...
		if(($fp = fopen($this->config->paths->root . "config.php", "w")) && fwrite($fp, $cfg)) {
			fclose($fp); 
			$this->ok("Saved configuration to ./config.php"); 
			return true; 
		} else {
			$this->err("Error saving configuration to ./config.php. Please make sure it is writable."); 
			return false;
		}
	}
	...
	protected function profileImport($database, array $options) {
		...
		$profile = $this->config->paths->site . "install/";
		...
		if(self::REPLACE_DB || !$result || $query->rowCount() == 0) {
			...
			$this->profileImportSQL($database, $this->config->paths->core . "install.sql", $profile . "install.sql", $options); 
			
			if(is_dir($profile . "files")) $this->profileImportFiles($profile);
				else $this->mkdir($this->config->paths->files); 
			
			$this->mkdir($this->config->paths->cache); 
			$this->mkdir($this->config->paths->logs); 
			$this->mkdir($this->config->paths->sessions); 
	...
	protected function profileImportFiles($fromPath) {
		...
		foreach($dir as $file) {
			...
			// make a copy, and that copy should be writable by the server
			$result = $this->copyRecursive($pathname, $this->config->paths->public . substr($this->config->urls->assets, strlen($this->config->urls->root)) . "$dirname/"); 

			if($result) $this->ok("Imported: $pathname => ./public/assets/$dirname/"); 
				else $this->err("Error Importing: $pathname => ./public/assets/$dirname/"); 
	...
	protected function getRemoveableItems($wire, $getMarkup = false, $removeNow = false) {
		...
		$isPost = $wire && $wire->input->post->remove_items !== null;
		...
		$items = array(
			'install-php' => array(
				...
				'path' => $this->config->paths->public . "install.php", 
			)
		);
	...
	protected function adminAccountSave($wire) {
		...
		$this->ok("Note that future runtime errors are logged to <b>/storage/logs/errors.txt</b> (not web accessible).");
		...
		if(is_writable($this->config->paths->siteModules)) \ProcessWire\wireChmod($this->config->paths->siteModules, true); 
		...
		if(!self::TEST_MODE) {
			file_put_contents($this->config->paths->assets . "installed.php", "<?php // The existence of this file prevents the installer from running. Don't delete it unless you want to re-run the install or you have deleted ./install.php."); 
		}
